  ...
  return (
    <>
      {(profile.mobile || profile.whatsapp) && (
        <div className="rounded-lg sm:rounded-xl bg-gray-50 p-3 sm:p-4 space-y-2 sm:space-y-3">
          <p className="text-xs font-bold text-gray-700 uppercase tracking-wide">{t("talents.contact")}</p>
          {profile.mobile && (
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-blue-100 flex items-center justify-center">
                <Phone className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600" />
              </div>
              <div className="min-w-0">
                <p className="text-xs text-gray-600">{t("talents.phone")}</p>
                <p dir="ltr" className="text-sm font-semibold text-gray-900 truncate">{profile.mobile}</p>
              </div>
            </div>
          )}
          {profile.whatsapp && (
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-green-100 flex items-center justify-center">
                <MessageCircle className="h-4 w-4 sm:h-5 sm:w-5 text-green-600" />
              </div>
              <div className="min-w-0">
                <p className="text-xs text-gray-600">{t("talents.whatsapp")}</p>
                <p dir="ltr" className="text-sm font-semibold text-gray-900 truncate">{profile.whatsapp}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div>
        <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-2 sm:mb-3">{t("talents.physicalAttributes")}</h3>
      ...
      {(profile.chest || profile.bust || profile.waist) && (
        <div>
          <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-2 sm:mb-3">{t("talents.measurements")}</h3>
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            {profile.chest && (
              <div className="rounded-lg bg-gray-100 p-2 sm:p-3 text-center">
                <p className="text-xs text-gray-600">{t("talents.chest")}</p>
                <p className="text-base sm:text-lg font-bold text-gray-900 mt-1">{profile.chest}</p>
            ...
            {profile.bust && (
              <div className="rounded-lg bg-gray-100 p-2 sm:p-3 text-center">
                ...
                <p className="text-base sm:text-lg font-bold text-gray-900 mt-1">{profile.bust}</p>
                <p className="text-xs text-gray-500">cm</p>
              </div>
            )}
            {profile.waist && (
              <div className="rounded-lg bg-gray-100 p-2 sm:p-3 text-center">
                ...
                <p className="text-base sm:text-lg font-bold text-gray-900 mt-1">{profile.waist}</p>
                <p className="text-xs text-gray-500">cm</p>
              </div>
            )}
          </div>
      ...
      {(profile.shoe_size || profile.tshirt_size || profile.pants_size || profile.jacket_size) && (
        <div>
          <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-2 sm:mb-3">{t("talents.sizes")}</h3>
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            {profile.shoe_size && (
              <div className="rounded-lg bg-gray-100 p-2 sm:p-3">
