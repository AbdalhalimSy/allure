...
export default function MediaUploader({
  ...
  const getPreviewUrl = (file: File | string): string | null => {
    if (typeof file === 'string') {
      ...
      return file.startsWith('http') ? file : `https://allureportal.sawatech.ae/storage/${file}`;
    }
  ...
  const fileName = value instanceof File ? value.name : value ? value.split('/').pop() : null;
              ...
              <div>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  <span className="text-[#c49a47] dark:text-[#e3c37b] hover:underline">
                    {t('account.profession.upload.clickToUpload') || 'Click to upload'}
                  </span>{' '}
                  {t('account.profession.upload.orDragDrop') || 'or drag and drop'}
                </p>
        ...
        <div className="relative border-2 border-gray-300 dark:border-white/20 rounded-xl overflow-hidden bg-white dark:bg-white/5">
          ...
          {type === 'audio' && (
            <div className="p-6 flex items-center gap-4">
              ...
              {value instanceof File && (
                <audio src={URL.createObjectURL(value)} controls className="flex-1" />
              )}
              {typeof value === 'string' && (
                <audio src={getPreviewUrl(value) || ''} controls className="flex-1" />
              )}
            </div>
          )}
          ...
          <div className="p-4 border-t border-gray-200 dark:border-white/10 flex items-center justify-between">
            ...
            <button
              ...
              disabled={disabled}
              className="ml-3 p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
