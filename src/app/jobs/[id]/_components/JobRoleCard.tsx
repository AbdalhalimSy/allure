...
export function JobRoleCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-3xl border border-gray-200/50 bg-white shadow-lg transition-all hover:shadow-2xl">
      {/* Gradient Top Border */}
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
            <h3 className="text-2xl font-bold text-gray-900 flex-1">{role.name}</h3>
            {role.has_applied && (
              <div className="inline-flex items-center gap-2 rounded-full bg-linear-to-r from-emerald-500 to-green-500 px-4 py-2 shadow-lg">
          ...
          <div className="flex flex-wrap gap-2">
            {role.call_time_enabled && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-linear-to-r from-indigo-50 to-purple-50 border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700">
                <Clock className="h-3.5 w-3.5" />
                {t("jobs.appliedJobs.details.callTimeEnabled") || "Call time required"}
              </span>
            )}
            {role.budget && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-linear-to-r from-green-50 to-emerald-50 border border-green-200 px-3 py-1.5 text-xs font-semibold text-green-700">
          ...
          {activeTab === "overview" && (
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="flex items-center gap-3 rounded-xl bg-linear-to-br from-gray-50 to-gray-100 p-4 border border-gray-200">
                  <div className="rounded-lg bg-linear-to-br from-[#c49a47] to-[#d4a855] p-2">
                    <User className="h-4 w-4 text-white" />
                  </div>
                  <div>
                    <p className="text-xs text-gray-600 mb-0.5">{t("jobs.jobDetail.gender")}</p>
                    <p className="font-semibold text-gray-900">{t(`filters.${role.gender}`)}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 rounded-xl bg-linear-to-br from-gray-50 to-gray-100 p-4 border border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">{role.payment_terms_days} days</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mt-8 space-y-3">
          <button
            onClick={onApply}
            disabled={isDisabled}
            className={`group/btn relative w-full overflow-hidden rounded-2xl px-8 py-4 font-bold text-lg shadow-xl transition-all ${
              isDisabled
