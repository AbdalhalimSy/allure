...
export default function AppliedJobsPage() {
  ...
  const fetchAppliedJobs = async (pageNum: number) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data.message || data.error || t("jobs.appliedJobs.errors.loadFailed")
        );
      }
      ...
      setError(
        err instanceof Error ? err.message : t("jobs.appliedJobs.errors.loadFailed")
      );
    } finally {
      setLoading(false);
    }
  ...
  const stats: StatItem[] = useMemo(() => {
    ...
    return [
      {
        key: "total",
        label: t("jobs.appliedJobs.stats.total"),
        count: total,
        color: "bg-gray-100 dark:bg-gray-800",
      },
      {
        key: "pending",
        label: t("jobs.appliedJobs.stats.pending"),
        count: pending,
        color: "bg-amber-50 dark:bg-amber-900/20",
      },
      {
        key: "shortlisted",
        label: t("jobs.appliedJobs.stats.shortlisted"),
        count: shortlisted,
        color: "bg-blue-50 dark:bg-blue-900/20",
      },
      {
        key: "selected",
        label: t("jobs.appliedJobs.stats.selected"),
        count: selected,
        color: "bg-emerald-50 dark:bg-emerald-900/20",
      },
      {
        key: "rejected",
        label: t("jobs.appliedJobs.stats.rejected"),
        count: rejected,
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 text-center">
        <div className="mx-auto w-fit rounded-2xl border border-dashed border-gray-200 bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          ...
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">
            {t("jobs.jobs.authRequired")}
          </h2>
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            {t("jobs.jobs.authRequiredDescription")}
          </p>
      ...
      <div className="mx-auto max-w-4xl px-6 py-12">
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-8 text-center dark:border-rose-900/40 dark:bg-rose-950/30">
          ...
          <h2 className="text-xl font-semibold text-rose-700 dark:text-rose-200">
            {t("jobs.appliedJobs.errors.loadFailed")}
          </h2>
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-[#c49a47]">
              {t("jobs.appliedJobs.title")}
            </p>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
              {t("jobs.appliedJobs.subtitle")}
            </h1>
          </div>
          <div className="flex items-center gap-2 rounded-full bg-gray-50 px-4 py-2 text-sm text-gray-600 dark:bg-gray-800 dark:text-gray-300">
            ...
            <span>
              {t("jobs.appliedJobs.stats.total")}:{" "}
          ...
          {stats.map((stat) => {
            ...
            return (
              <button
                ...
                className={`${
                  stat.color
                } w-full rounded-xl border border-gray-200/70 p-4 text-start transition hover:-translate-y-0.5 hover:shadow dark:border-gray-800 ${
                  isActive
                    ? "ring-2 ring-[#c49a47] border-[#c49a47]/60 shadow-md"
                    : ""
                }`}
              >
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-10 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            {t("jobs.appliedJobs.noApplications")}
          </h3>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {search
              ? t("jobs.appliedJobs.noSearchResults")
              : t("jobs.appliedJobs.noApplicationsMessage")}
          </p>
                ...
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                      {t("jobs.appliedJobs.details.appliedOn")}:{" "}
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  <div className="rounded-lg bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {t("jobs.appliedJobs.details.ageRange")}
                    </p>
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {role.start_age} - {role.end_age}{" "}
                      {t("jobs.appliedJobs.details.years")}
                    </p>
                  </div>
                  <div className="rounded-lg bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {role.gender ? t(`filters.${role.gender}`) : "—"}
                    </p>
                  </div>
                  <div className="rounded-lg bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {t("jobs.appliedJobs.details.paymentTerms")}
                    </p>
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {role.payment_terms_days} {t("jobs.appliedJobs.details.days")}
                    </p>
                  </div>
                  <div className="rounded-lg bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {t("jobs.appliedJobs.details.ethnicity")}
                    </p>
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {role.ethnicity?.join(", ") || "-"}
                    </p>
                  ...
                  {role.call_time_enabled && (
                    <div className="rounded-lg bg-indigo-50 p-3 text-sm text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-100">
                      ...
                      <p className="mt-1 text-sm">
                        {t("jobs.appliedJobs.details.callTimeMessage")}
                      </p>
                    </div>
                  )}
                </div>
