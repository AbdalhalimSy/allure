...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/profile/${profileId}/portfolio`, {
      method: "GET",
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.message || "Failed to fetch portfolio" }, { status: response.status });
    }
