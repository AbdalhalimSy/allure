...
export async function POST(request: NextRequest) {
  return handleAuthenticatedRequest(request, async ({ token, profileId, request: req }) => {
    ...
    if (!contentType.includes("multipart/form-data")) {
      return errorResponse("Content-Type must be multipart/form-data for syncing portfolio", 400);
    }
    ...
    const response = await fetchFromBackend(req, token, "/profile/sync-portfolio", {
      method: "POST",
      body: formData,
      multipart: true,
    });
