...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!contentType.includes("multipart/form-data")) {
      return NextResponse.json(
        { error: 'Content-Type must be multipart/form-data for syncing experiences.' },
        { status: 400 }
      );
    }
  ...
  delete headers["Content-Type"];
    ...
    const response = await fetch(`${BACKEND_URL}/profile/sync-experiences`, {
      method: "POST",
      headers,
      body: formData,
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.message || "Failed to sync experiences" },
